...
// variável é um nome símbolico para um valor
// As variáveis são declaradas com a palavra-chave var:
var x; // Declara uma variável chamada x.

// Valores pode ser atribuídos às variáveis com o sinal =
x = 0; // Agora a variável x tem o valor 0
x; // => 0: Uma variável é avaliada com seu valor.

// JavaScript aceita vário tipos de valores
x = 1; // Números.
x = 0.01; // Apenas um tipo Number para inteiros e rais.
...
book["fat"]; // => true: outro modo de acessar valores de propriedades.
...
var data = {
  ...
  trail2: [[2, 3], 4.5], // Os elementos dos arrays são arrays.
};

// Os operadore atuam sobre os valores (operandos) para produzir um novo valor.
// Os oepradores aritméticos são os mais comuns:
3 + 2; // => 5: adição
3 - 2; // => 1: subtração
3 * 3; // => 6: multiplicação
3 / 3; // => 1.5: divisão
...
function plus1(x) {
  // Define uma função chama "plus1", com o parâmetro "x"
...
points.dist = function () {
  // Define um método para calcular a distância entre pontos
  ...
  return Math.sqrt(a * a + b * b); // O terorema de Pitágoras
};
points.dist(); // => 1,414: distância entre nossos 2 pontos

// As intruções JavaScript incluem condicionais e laços que usam a sintaxe das linguaguens C, C++, Java e outras.
function abs(x) {
  ...
  if (x >= 0) {
    ...
    // A cláusula opcional else executa seu código se a comparação for falsa.
...
function Point(x, y) {
  // Por convenção, as construtoras começam com letras maíusculas
